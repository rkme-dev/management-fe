...
<script>
import { mdiAccountEdit, mdiCancel, mdiDeleteCircle } from '@mdi/js'
import {
  ref,
  watch,
} from '@vue/composition-api'
import TermForm from '@/views/pages/master-files/terms/TermForm.vue'
...
export default {
  ...
  setup() {
    ...
    watch(filter, value => {
      store.dispatch('TermStore/filter', filter.value)
    })
  ...
  computed: {
    ...
    terms() {
      return this.$store.state.TermStore.terms
    },
    loading() {
      return this.$store.state.TermStore.loading
    },
  },
  ...
  created() {
    this.$store.dispatch('TermStore/list')
    this.$store.dispatch('TermStore/removeErrors')
  },
  methods: {
    ...
    deleteItemConfirm() {
      this.terms[this.editedIndex].is_active = false
      store.dispatch('TermStore/update', this.terms[this.editedIndex]).then(
        response => {
